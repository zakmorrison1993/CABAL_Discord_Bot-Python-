...
#   date = time.localtime(clock) # establish date    
 #  daystogo = 261 - int(date.tm_yday) # daystogo to event minus the date
  # if daystogo > 1:
  #  text = "```"+str(daystogo) + " Days to go till the Zero Hour Anniversary!!```" # text
  # if daystogo == 0:
  #  text = "``` Today is the day: Zero Hour Anniversary!!```" # text
...
#  single run time anniversary event note
  ...
  if message.content.startswith('$whatson'): # if user says "$music"
     await message.channel.send(file=discord.File('Zero_Hour.jpg')) # send poster
 ...
 # show prize pool
  if message.content.startswith('$prize'): # if user says "$prize"
     ...
     text3 = "https://cnc.community/generals/how-to-play"
     await message.channel.send(text) # discord sends data back
...
# cast music vote
  if message.content.startswith('$v_'): # vote command
     ...
     if matching: # if true
        ...
        await message.channel.send(text) # send messag

#
#
#
# library - unit search up
#
# 
#  
  ...
  if message.content.startswith('$library'): # if user says "$search"
     input = message.content # input - discord message
     my_list = list(input) # add to list
     edit = "" # host variable

     skipped_list = [x for i, x in enumerate(my_list) if i > 8] # take everything after the 7th character
     for a in skipped_list: # loop through list
         edit+=a # add to string
  
     my_file = open("library.txt","r")
     content = my_file.readlines()
     my_file.close()

     item = [s for s in content if edit in s]
...
#
  ...
  if message.content.startswith('$8ball'): # if user says "$8ball"
     ...
     await message.channel.send(file=discord.File(''+str(num)+'.png'))
 
   # spice
  if message.content.startswith('$spice'): # if user says "$spice"
     await message.channel.send(file=discord.File('spice.gif'))

   # fortune
  if message.content.startswith('$fortune'): # if user says "$fortune"
     my_file = open("fortune.txt","r")
     content = my_file.readlines()
     my_file.close()
