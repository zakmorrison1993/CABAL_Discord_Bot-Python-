...
async def anniversary(): # def
   ...
   daystogo = 273 - int(date.tm_yday) # daystogo to event minus the date
...
# shows avaliable streams
  if message.content.startswith('$stream'): # if user says "$download_tibsun"
     ...
     my_file = open("misc\streamers.txt", "r") # open streamer text - this will need optermising for hosting
...
# display random cnc song
  if message.content.startswith('$music'): # if user says "$music"
     my_file = open("misc\music.txt", "r") # open music list
...
#
#
# Anniversary commands - commands designed speficially for Tib Sun/ZH Anniversary
#
#
#
#
...
#  single run time anniversary event note
  if message.content.startswith('$time'): # message starts with "time"
   ...
   daystogo = 273 - int(date.tm_yday)
   text = "```"+str(daystogo) + " Day to go till Zero Hours 18th Anniversary!!```" # text
   await message.channel.send(text) # discord sends data back

   # display poster for Tiberium Sun Anniversary event
  if message.content.startswith('$whatson'): # if user says "$music"
     await message.channel.send(file=discord.File('zh\Zero_Hour.jpeg')) # send poster

 # show prize pool
  if message.content.startswith('$prize'): # if user says "$prize"
     pounds = math.floor(prizepool - (prizepool*0.38)) # covert to pounds
     euros = math.floor(prizepool +- (prizepool*0.18)) # convert to euros
     text = "```Current Prize Pool is : $: " + str(prizepool) + "   £: " + str(pounds) + "   EU: " + str(euros)+"```" # text 1
     text2 = "``` Details: 18 September (Saturday): Official Zero Hour 18th Anniversary,  (Tournament hosted by GameReplays) ```" # text 2
     text3 = "https://cnc.community/generals/how-to-play # text 3"
     await message.channel.send(text) # discord sends data back
 ...
 # add to prizepool
  if message.content.startswith('$pool'): # if user says "$pool"
     ...
     for a in new.split(): # split list
        if a.isdigit(): # check if list element is digit
           edit.append(a) # append digits to host variable
  ...
  if message.content.startswith('$donate'): # if user says "$download_kw"
     text = "To donate to the tournament please goto : https://www.paypal.com/paypalme/zerohourgr" # text
     await message.channel.send(text) # discord sends data back 
...
# cast music vote
  if message.content.startswith('$v_'): # vote command
     my_file = open("tibsun\musicvote.txt", "r") # open music vote file
     ...
     if matching: # if true
        ...
        my_file = open("tibsun\musicvote.txt", "a") # open music vote file
...
# 
  ...
  if message.content.startswith('$list_mods'): # if user says "$list_mods"
    my_file = open("misc\modlist.txt", "r") # open modlist  
...
#
  ...
  if message.content.startswith('$discord'): # if user says "$download_tibsun"
     my_file = open("misc\discord.txt", "r") # open discord list
     content = my_file.read() # set content to read file
     my_file.close() # close file
     print(content) # print contents
     await message.channel.send(content) # send contents
  ...
  if message.content.startswith('$help'): # if user says "$help"
    my_file = open("menus\menu_main.txt", "r") #open help text
    content = my_file.read() # set content to read file
    my_file.close() # close file
    print(content) # print contents
    await message.channel.send(content) # send contents
  ...
  if message.content.startswith('$cnconline'): # if user says "$help"
    my_file = open("cnconline\cnco_menu.txt", "r") #open help text
    content = my_file.read() # set content to read file
    my_file.close() # close file
    print(content) # print contents
    await message.channel.send(content) # send contents
  ...
  if message.content.startswith('$mods'): # if user says "$help"
    my_file = open("menus\mods_menu.txt", "r") #open help text
    content = my_file.read() # set content to read file
    my_file.close() # close file
    print(content) # print contents
    await message.channel.send(content) # send contents
  ...
  if message.content.startswith('$htp'): # if user says "$help"
    my_file = open("menus\htp_menu.txt", "r") #open help text
    content = my_file.read() # set content to read file
    my_file.close() # close file
    print(content) # print contents
    await message.channel.send(content) # send contents
...
#
  if message.content.startswith('$findmatch'): # if user says "$findmatch"
    ...
    my_file = open("tibsun\tibsunautomatch.txt", "r") # open auto match file
    ...
    if len(content) > 0: # if value
         ...
         my_file = open("tibsun\tibsunautomatch.txt", "w") # open file
...
#
  ...
  if message.content.startswith('$edvin'): # if user says "$edvin"
    my_file = open("edvin\edvin.txt", "r") #open help text
    content = my_file.read() # set content to read file
    my_file.close() # close file
    print(content) # print contents
    await message.channel.send(content) # send contents 
...
#
  ...
  if message.content.startswith('$8ball'): # if user says "$8ball"
     ...
     await message.channel.send(file=discord.File('8ball/'+str(num)+'.png'))
 
   # fortune
  if message.content.startswith('$fortune'): # if user says "$fortune"
     my_file = open("misc\fortune.txt","r")
     content = my_file.readlines()
     my_file.close()
     text = content[random.randint(1,len(content))]   
     await message.channel.send(text) # discord sends data back 
...
#
#
  if message.content.startswith('$game'): # if user says "$nyan"
     # take data
     input = message.content
     my_list = list(input) # add to list
     edit = "" # host variable
     skipped_list = [x for i, x in enumerate(my_list) if i > 5] # take everything after the 7th character
     for a in skipped_list: # loop through list
         edit+=a # add to string
 # Rock, Paper and Scissors
     if edit == "rock" or edit == "paper" or edit == "scissors":
        answer = ["","Rock", "Paper", "Scissors"]
        text = str(answer[random.randint(1,3)])
        if (edit == "rock" and text == "Paper") or (edit == "paper" and text == "Scissors") or (edit == "scissors" and text == "Rock"):
          text2 = "You Lose!"
          text3 = "HAHAHA!"
        if (edit == "rock" and text == "Scissors") or (edit == "paper" and text == "Rock") or (edit == "scissors" and text == "Paper"):
          text2 = "You Win!"
          text3 = "Unforgivable"
        if (edit == "rock" and text == "Rock") or (edit == "paper" and text == "Paper") or (edit == "scissors" and text == "Scissors"):
          text2 = "We Draw!"
          text3 = "DAMN!"
        await message.channel.send(text) # discord sends data back
        await message.channel.send(text2) # discord sends data back
   ...
   # BETA Rock, Paper and Scissors
     if edit == "b_rock" or edit == "b_paper" or edit == "b_scissors":
          global id_score
          id_score = [message.author.id, 0,0,0]
          global gamesnum
          gamesnum = +1
          text = "Now playing Rock, Paper and Scissors with<@"+str(message.author.id)+">! type end to quit."
          await message.channel.send(text) # discord sends data back
    
    
    
  if gamesnum > 0:
     if message.author.id == id_score[0]:
      edit = message.content  
      if edit == "rock" or edit == "paper" or edit == "scissors" or edit == "end":
        answer = ["","Rock", "Paper", "Scissors"]
        text = str(answer[random.randint(1,3)])
        if text == edit:
           text = str(answer[random.randint(1,3)])
        ...
        if (edit == "rock" and text == "Rock") or (edit == "paper" and text == "Paper") or (edit == "scissors" and text == "Scissors"):
          ...
          text3 = "DAMN!"
        if edit == "end":
           text = "Thank you for playing <@"+str(message.author.id)+">!!!"
           text2 = "Wins: "+str(id_score[1])+" , Loses: "+str(id_score[2])+" , Draws: "+str(id_score[3])+" !!!!"
           if id_score[1] > id_score[2] and id_score[1] > id_score[3]:
              text3 = "Next time you wont be so lucky, Commander!"
           if id_score[2] > id_score[1] and id_score[2] > id_score[3]:
              text3 = "Observe suprior tactics while you still have human eyes!"
           if id_score[3] > id_score[2] and id_score[3] > id_score[1]:
              text3 = "This is embarresing for both of us..." 
           id_score[0] = 0
           gamesnum =- 1   
